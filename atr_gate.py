...
trader silent-absorbs the entry (no order, no unit). Other codes
unaffected. Missing ATR → fail-open (no skip), so a Worker bug can never
kill all ④ signals.
"""


def should_skip_code4_atr(sig_code, sig_atr, threshold):
